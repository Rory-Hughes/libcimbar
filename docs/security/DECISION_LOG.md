...
## ADR-LC-029: Use a fixed OIP-to-SCP IPC envelope

- **Status:** Accepted
- **Context:** WP-12 required a smaller, auditable boundary between optical ingress, the sandboxed decoder, and the Secure Core. The decoder process must not send filenames, paths, pointers, nested application objects, or arbitrary diagnostics back into the trusted domain after reconstructing hostile optical input.
- **Decision:** Add `HardenedTransportIpc`, a dependency-free parser for a single fixed 24-byte little-endian envelope with magic `LCIP`, exact version 1, fixed message type, zero reserved flags, nonzero transfer generation, exact payload size, and fixed status code. Only `submit_frame` and `completed_object` may carry opaque payload bytes. Control, status, and failure messages must be zero-payload. The parser accepts caller-selected full-message, frame-payload, and object-payload limits and returns valid payloads only as an offset and length into caller-owned input.
- **Reason:** A fixed binary envelope is easier to review and fuzz than reusing the optical protocol, JSON, protobuf, filenames, textual diagnostics, or a process-local object API across the trust boundary. Generation scoping lets the Secure Core drop stale messages before acting on bytes from a restarted decoder child.
- **Alternatives considered:** Reuse fountain metadata, pass completed objects through filesystem paths, send JSON diagnostics, or wait for the final service implementation. Fountain metadata lacks reset/generation/status semantics for process IPC, paths would reintroduce filesystem authority, JSON/text requires a larger parser and diagnostic sanitization, and waiting would leave the sandbox prototype without an enforced message contract.
- **Security consequences:** Unsupported versions, unknown types, reserved flags, generation zero, length mismatches, unknown status codes, over-limit payloads, missing required payloads, and payloads on control/status/failure messages fail closed. The format has no pointer, filename, nested object, path, MIME type, application command, wallet instruction, or free-form diagnostic field. Parser fuzzing is independent of OpenCV, Reed-Solomon, Wirehair, zstd, filesystem, and sandbox code.
- **Compatibility consequences:** No existing compatibility API changes. Future OIP/SCP integration must map local logging and richer operator diagnostics outside this IPC payload rather than extending decoder-originated messages with strings.
- **Validation required:** Build the hardened transport profile, run the IPC unit coverage in `hardened_transport_profile_test`, regenerate/check the deterministic corpus, build the standalone libFuzzer workspace, run the `fuzz_hardened_ipc` corpus smoke under ASan/UBSan, and keep the target in sanitizer CI.
